...
def scoring(a, b):
    global file1, file2, tempp, minarrdd, huling, support
    xv = []
    yv=[]
    hull = ConvexHull(a)
    ...
    z = pathe.contains_points([(i[0], i[1]) for i in file2])
    for i in z:
        if i == True:
            score = score + 1
...
class Nn(App):
    ...
    def build(self):
        ...
        for i, k in zip(range(len(huling)), range(len(dur))):
           for simplex in huling[i]:
                ax.plot(np.array(dur[k])[simplex, 0], np.array(dur[k])[simplex, 1], marker='o', linestyle='-', markerfacecolor="blue",color='red')
    ...
    def specific(self,index,unused):
        ...
        for j in huling[index]:
            ax2.plot(np.array(dur[index])[j,0], np.array(dur[index])[j,1], marker='o', linestyle='-', markerfacecolor="blue",color='red')
...
def nearest(a,b,c,d,e):
        global file1,file2,tempp,minarrdd,huling, support,file3,file4,dur
        file1=a
        file2=b
        file3=c
        file4=e
        ...
        for k in range(len(a)):
                ...
                for i in range(len(indices)):
                    for j in range(len(indices)):
                            threshold = (min(t[i]) + max(t[i])) / len(X)
                            if j != 0 and abs(t[i][0] - t[i][j]) <= float(d):
                                if indices[i][0] not in tem1 and indices[i][0] not in tem2:
                                    tem1.append(indices[i][0])
                                    tem2.append(indices[i][j])
                                    prune.remove(X[indices[i][0]])
                if len(prune) > 2:
                    for i in range(len(prune)):
                            root = Node(prune[i])
                            for j in range(len(prune)):
                                if i != j:
                                    root.add_child(prune[j])
